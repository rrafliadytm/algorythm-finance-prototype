  ...
  return (
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="no-scrollbar flex-1 space-y-5 overflow-y-auto px-5 pb-10 pt-1">
        {/* Connection cards */}
        <div>
          <p className="mb-2 text-xs font-semibold text-muted-foreground">Dompet Tersinkronisasi</p>
            ...
            {wallets.map((w, i) => (
              <button
                ...
                className={cn(
                  ...
                  selected === i ? 'border-primary/50 bg-primary/10 glow-green' : 'border-border bg-card',
                )}
              >
                <div
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-muted-foreground tabular-nums">{w.phone}</p>
                </div>
                {selected === i && <CheckCircle2 className="h-5 w-5 text-primary" />}
              </button>
            ))}
            <button
              type="button"
        ...
        <Card>
          ...
          <div className="mt-3 flex items-center justify-between rounded-xl bg-primary/10 px-4 py-3">
            <span className="text-xs text-muted-foreground">Saldo diterima</span>
            <span className="text-lg font-black text-primary text-glow-green">Rp{balance}</span>
          </div>
        ...
        <p className="text-center text-[11px] text-muted-foreground text-pretty">
          Dilindungi otentikasi ganda (Biometrik / PIN) untuk mencegah fraud finansial.
        </p>
      </div>
    </div>
  )
